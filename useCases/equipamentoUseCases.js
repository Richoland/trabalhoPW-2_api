...
const getEquipamentosPorSalaDB = async (codigosala) => {
    try {
        const results = await
            pool.query(`SELECT * FROM equipamentos WHERE sala = $1 
        ORDER BY codigo`, [codigosala]);
        if (results.rowCount === 0) {
            throw `Nenhum equipamento encontrado com o código de 
            sala: ${codigosala}`;
        } else {
            return results.rows.map((equipamento) =>
                new Equipamento(equipamento.codigo, equipamento.descricao,
                    equipamento.numero_serie, equipamento.valor,
                    equipamento.sala));
        }
    } catch (err) {
        throw "Erro: " + err;
...
const addEquipamentoDB = async (body) => {
    try {
        ...
        const results = await pool.query(`INSERT INTO equipamentos (descricao,
             numero_serie, valor, sala) VALUES ($1, $2, $3, $4) 
...
const updateEquipamentoDB = async (body) => {
    try {
        ...
        const results = await pool.query(`UPDATE equipamentos SET 
        descricao=$1, numero_serie=$2, valor=$3, sala=$4 WHERE codigo=$5 
        RETURNING codigo, descricao, numero_serie, valor, sala`,
...
module.exports = {
    getEquipamentosPorSalaDB, addEquipamentoDB, updateEquipamentoDB, 
